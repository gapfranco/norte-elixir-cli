...
import { showMapping, createMapping, updateMapping, deleteMapping } from '~/src/services/mappingsApi'
...
class EditMapping extends React.Component {
  ...
  componentDidMount () {
    ...
    if (id === '+') {
      this.setState({
        data: { item_key: this.props.match.params.key, unit_key: null, user_key: null },
        id: id
      })
    } else {
      showMapping(id)
        .then(res => {
          this.setState({
            data: { item_key: this.props.match.params.key,
  ...
  getOptions = async () => {
    ...
    this.setState({ units: res1.data.data.units.list, users: res2.data.data.users.list, loaded: true })
  }
  ...
  handleUnit = value => {
    this.setState({ data: { ...this.state.data, unit_key: value } }, () => console.log(this.state.data))
  }

  handleUser = value => {
    this.setState({ data: { ...this.state.data, user_key: value } }, () => console.log(this.state.data))
  }
